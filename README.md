...
[![Gates](https://img.shields.io/badge/gates-97%20core%20%2B%207%20display-brightgreen.svg)](scripts/run-all-tests.cjs)
...
| 门禁 | `npm run verify` = 构建 + `check:js`（主进程/脚本 checkJs）+ **97 项核心套件 + 7 项显示环境套件**；清单唯一来源 `scripts/run-all-tests.cjs` |
...
src/
  ...
  types.ts          节点数据类型
scripts/            门禁与测试套件（97 项核心 + 7 项显示）/ 打包 / 发布 / 评测
```
