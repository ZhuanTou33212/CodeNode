...
- 节点程序模式：复用节点图并提交可执行产物意图；Stage0 只冻结协议和本地申请闭环，封装代码节点、完整程序转译与编译运行留到后续阶段。
- Markdown 请求模式：选择单个或全部节点，把节点 Prompt 与连接结构输出为 `.md` 制作请求；语言选择用于加载对应语言的规划约束，本阶段不生成代码、不编译运行。
