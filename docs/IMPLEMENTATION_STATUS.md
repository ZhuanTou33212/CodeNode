# CodeNode Desktop 已实现功能清单（Stage 0 → 4.8）

> 生成时间：2026-08-10
> 基准：`codenode-desktop` 工作树（HEAD `99ec7b5`，Stage4.8），含当前未提交改动
> 验证：`mvnw test` **136 项全部通过**（20 个测试类）；pom 版本 `0.1.3`，应用自报 `0.5.0`
...
| Stage 4.7 | 实时运行追踪（工程构建运行 + JFR 追踪） | ◑ 部分实现 |
| Stage 4.8 | 文件浏览器 + 多文档 tab + 右侧面板可拖拽 + 工程构建运行 | ◑ 部分实现 |
| Stage 4.9 | Agent 全权限 + 上下文注入 + 知识图谱/长期记忆 + 子代理/任务清单 + .cnode 预留空间（已并入原 4.10） | 📋 仅方案（STAGE_4_9_OUTLINE.md） |
