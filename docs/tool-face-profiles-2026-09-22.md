...
## 8.7 P1-4 收尾：先 compact，再做不可逆硬裁剪（同日）

审计第 306/328 行点名的是一个**时序**问题：硬裁剪把旧工具结果正文换成占位符是**不可逆**的，
而旧流程里「字符预算超了」这一轮**直接换占位符**，语义压缩要到下一轮才触发 ——
那时摘要看到的**已经是占位符**，原始正文再也回不来了。

改法（`runCompactionStep` + 主循环）：

1. 动手裁之前**先探一次**「这一轮到底会不会丢正文」（`contextBudget.planTrim` 是纯函数，零成本、不改消息）；
2. 会丢 → 先做语义压缩，触发来源记 `before-trim`（`compaction_done` 事件新增 `trigger` 字段，可回放归因）；
3. 压缩成功 → 历史已被摘要替换，通常就没得裁了（原始正文进了摘要）；
4. 压缩失败 / 不可用 / **不划算**（P1-3 的 ROI 门：只剩一轮就别付这笔钱）→ 才真的换占位符，
   并把 `lastTrimStats` 留给下一轮再试一次压缩（原有的 `after-trim` 补救路径与硬裁剪兜底网都没动）。

判据（`test:compaction-tail` 新增 G 组，端到端真实 run）：
- 硬裁剪本来会丢正文时 → `trigger === 'before-trim'`（不是等下一轮的 `after-trim`）；
- **摘要请求看到的是原始正文**（不是占位符）—— 这正是旧时序坏掉的地方；
- 压完主请求里既没有原始正文（进了摘要）也没有占位符（没做不可逆裁剪）；
- **负向**：把压缩关掉 → 硬裁剪照旧兜底（占位符出现、`context_trim` 计数 > 0），兜底网没被拆掉。
