...
## 13. 子代理继承收紧（A4）+ 收紧归因（2026-09-21）

### 13.1 A4：`fork` 必须继承意图收紧

**缺口**：子代理的工具上下文是 `context.fork({...})` 出来的，而 `fork` **没有**带上
`intentPolicyValue` / `intentReviewer` —— 于是「换一个上下文」就绕过了收紧：子代理的写类动作既不复核，
它的审批也不受轮级判定影响。

**修法**：`fork` 继承这两项（`electron/tools/context.cjs`）。语义上这更严格也更正确 ——
**子代理动作的授权来源是用户对主任务的授权（用户说过的话），而不是主代理给子代理的任务描述**
（后者是 assistant 生成的东西，属**不可信证据**；子代理尤其容易被它读到的文件内容诱导）。

已知取舍：① 子代理复核**共享**主 run 的动作预算（总量仍有上限，预算耗尽时子代理同样按「没有信号」回落，
不收紧也不放宽）；② 按值继承 —— 主 run 之后的重判**不会**回灌到已 fork 的子上下文（子代理生命周期短，可接受）。

### 13.2 收紧归因（用户要知道「为什么又问一次」）

动作级收紧会让**本来不需要确认**的工具弹确认框。如果弹窗只写工具描述，用户看到的就是「凭空多问一次」。
现在 `registry` 在收紧时给确认说明追加一句归因（`（意图复核：本轮动作被判定为「风险高 / 授权不明 / 置信低」…）`），
**不收紧时不追加**（否则每次确认都像被意图拦了）—— 正反两个方向都有判据。

### 13.3 判据与变异

- `test:intent-action` **31 → 37 条**（新增 E 组 3 条 fork 继承 + 归因正负对照 3 条）；
- 变异 **29/29 → 32/32、0 skip**（新增：fork 不继承 / 归因缺失 / 归因无条件加）。
- 顺带记一个契约坑：`requiresConfirmation` 的合法取值是**大写**的 `ConfirmationLevel`（`'WRITE'`），
  写成小写会被 `normalizeDescriptor` **静默归一成 `false`**（本轮的测试 fixture 一开始就栽在这里，
  症状是「声明了要确认，结果一次都没问」）。
