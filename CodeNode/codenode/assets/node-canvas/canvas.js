(() => {
  ...
  document.querySelector('#send-request').addEventListener('click', async () => { const node = nodeById(state.selected); if (!node) { inspector.validation.textContent = '请先选择一个节点。'; return; } inspector.validation.textContent = '正在发送到 CodeNode MCP…'; try { const response = await fetch('http://127.0.0.1:32145/markdown', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CodeNode-Bridge': '1' }, body: JSON.stringify({ filename: `${node.id}-${Date.now()}.md`, content: markdownRequest(node) }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || `HTTP ${response.status}`); node.status = '已提交到 Codex MCP'; inspector.validation.textContent = `已提交 ${result.filename}。请在 Codex 对话中输入“处理最新 CodeNode 请求”。`; render(); } catch (error) { inspector.validation.textContent = `MCP 未连接：${error.message}。请安装并重启 CodeNode 插件。`; } });
