(() => {
  ...
  const state = { nodes: [], edges: [], selected: null, scale: 1, offset: { x: 0, y: 0 }, connecting: null, panning: null, saveHandle: null };
  let buildMode = 'node';
  let language = 'java';
  state.nodes = [];
  state.edges = [];
  ...
  function safeFileName(name, extension) { const cleaned = String(name || 'CodeNode输出').replace(/[<>:"/\\|?*\x00-\x1F]/g, '_').trim() || 'CodeNode输出'; return `${cleaned}.${extension}`; }
  function downloadFile(fileName, content, type) { const blob = new Blob([content], { type }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = fileName; link.style.display = 'none'; document.body.appendChild(link); link.click(); link.remove(); setTimeout(() => URL.revokeObjectURL(link.href), 1000); }
  async function saveProgramFile(fileName, content, extension) { if (!state.saveHandle) { downloadFile(fileName, content, 'text/plain;charset=utf-8'); return false; } const writable = await state.saveHandle.createWritable(); await writable.write(content); await writable.close(); return true; }
  async function buildSelected(mode) { try { const node = nodeById(state.selected); if (!node) { inspector.validation.textContent = '请先选择一个空白节点。'; return; } node.language = language; const text = node.prompt || ''; if (mode === 'program') { const source = programSource(node); const extension = language === 'powershell' ? 'ps1' : 'java'; const chosen = await saveProgramFile(safeFileName(node.name, extension), source, extension); inspector.validation.textContent = chosen ? `程序已写入选择的位置（${languageLabel(language)}）。` : '浏览器未提供文件句柄，已下载程序文件。'; return; } if (/相加|加法|add/i.test(text)) { node.name = node.name === '空白节点' ? '整数相加' : node.name; node.category = '转换'; node.inputs = [{ id: 'left', name: '左值', dataType: 'int' }, { id: 'right', name: '右值', dataType: 'int' }]; node.outputs = [{ id: 'result', name: '结果', dataType: 'int' }]; } else { node.category = '自定义'; node.inputs = [{ id: 'input', name: '输入值', dataType: 'int' }]; node.outputs = [{ id: 'output', name: '输出值', dataType: 'int' }]; } node.status = '代码节点'; node.code = generatedCode(node); downloadFile(safeFileName(node.name, 'md'), `# ${node.name}\n\n- 语言：${languageLabel(language)}\n- 类型：${node.category}\n- 制作要求：${node.prompt || '未填写'}\n\n## 代码\n\n\`\`\`${language === 'powershell' ? 'powershell' : 'java'}\n${node.code}\n\`\`\`\n`, 'text/markdown;charset=utf-8'); inspector.validation.textContent = '节点 Markdown 已下载，当前节点已具备代码功能。'; render(); } catch (error) { inspector.validation.textContent = `输出失败：${error.message || error}`; console.error(error); } }
  document.querySelector('#choose-location').addEventListener('click', async () => { try { if (!window.isSecureContext || typeof window.showSaveFilePicker !== 'function') { state.saveHandle = null; document.querySelector('#location-status').textContent = '当前环境将使用下载'; return; } state.saveHandle = await window.showSaveFilePicker({ suggestedName: 'CodeNode程序.txt', types: [{ description: 'CodeNode 输出文件', accept: { 'text/plain': ['.java', '.ps1', '.md', '.txt'] } }] }); document.querySelector('#location-status').textContent = '已选择保存位置'; inspector.validation.textContent = '位置已选择，现在可以选择制作方式。'; } catch (error) { if (error.name !== 'AbortError') inspector.validation.textContent = `选择位置失败：${error.message || error}`; } });
  document.querySelector('#build-select').addEventListener('change', async event => { const mode = event.target.value; event.target.value = ''; if (mode) await buildSelected(mode); });
